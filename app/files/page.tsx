    ...
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 20px 60px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={loadFiles} disabled={loading} style={{ padding: '8px 14px', background: '#1a1d27', border: '1px solid #2a2d3a', borderRadius: 8, color: '#94a3b8', fontSize: 13, cursor: 'pointer' }}>
            {loading ? '...' : '↻ Refresh'}
          </button>
      ...
      {loading && files.length === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 0', color: '#475569', fontFamily: "'DM Mono', monospace" }}>Loading files...</div>
      )}
      ...
      {files.map(file => {
        ...
        return (
          <div key={file.key} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: '#1a1d27', border: '1px solid #2a2d3a', borderRadius: 12, marginBottom: 8 }}>
            {/* Icon */}
            ...
            <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
              <button
                ...
                disabled={isDl}
                style={{ padding: '8px 14px', background: isDl ? '#1e3a5f' : '#1e3a5f', border: '1px solid #2563eb', borderRadius: 8, color: isDl ? '#64748b' : '#93c5fd', fontSize: 13, cursor: isDl ? 'default' : 'pointer', fontWeight: 500, whiteSpace: 'nowrap' }}
              >
                {isDl ? '...' : '↓ Download'}
              </button>
              <button
