...
function QRCode({ url }: { url: string }) {
  ...
  return (
    <div style={{ textAlign: 'center', padding: '20px', background: '#1a1d27', border: '1px solid #2a2d3a', borderRadius: 12, marginBottom: 20 }}>
      <p style={{ fontSize: 11, color: '#64748b', fontFamily: 'monospace', marginBottom: 10, letterSpacing: '0.08em', textTransform: 'uppercase' as const }}>Scan to open on mobile</p>
      <img src={qrUrl} alt="QR Code" width={120} height={120} style={{ borderRadius: 8, display: 'block', margin: '0 auto' }} />
      <p style={{ fontSize: 11, color: '#475569', fontFamily: 'monospace', marginTop: 10, wordBreak: 'break-all' as const }}>{url}</p>
    </div>
  )
}
...
export default function Home() {
  ...
  async function uploadAll() {
    ...
    for (const entry of pending) {
      try {
        setEntries(prev => prev.map(e => e.id === entry.id ? { ...e, progress: 5 } : e))

        const ext = entry.file.name.includes('.') ? '.' + entry.file.name.split('.').pop() : ''
      ...
      <div style={{ width: '100%', maxWidth: 380 }}>
        ...
        <div style={{ background: '#1a1d27', border: '1px solid #2a2d3a', borderRadius: 16, padding: '28px 24px' }}>
          ...
          <input type="password" value={pw} onChange={e => setPw(e.target.value)}
            ...
            placeholder="Enter portal password" autoFocus
            style={{ width: '100%', padding: '12px 14px', background: '#0f1117', border: `1px solid ${pwError ? '#7f1d1d' : '#2a2d3a'}`, borderRadius: 10, color: '#e2e8f0', fontSize: 15, outline: 'none', boxSizing: 'border-box' as const }} />
          {pwError && <p style={{ color: '#f87171', fontSize: 13, marginTop: 8, fontFamily: 'monospace' }}>{pwError}</p>}
          <button onClick={login} disabled={checking || !pw}
            style={{ width: '100%', marginTop: 14, padding: '13px', background: checking ? '#1e3a5f' : '#2563eb', color: '#fff', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 600, cursor: checking ? 'default' : 'pointer' }}>
  ...
  return (
    <div style={{ maxWidth: 600, margin: '0 auto', padding: '32px 20px 60px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
          {[
            ...
            <button key={label} onClick={() => (ref as React.RefObject<HTMLInputElement>).current?.click()}
              ...
              style={{ padding: '16px 8px', background: '#1a1d27', border: '1px solid #2a2d3a', borderRadius: 12, color: '#e2e8f0', cursor: 'pointer', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: 6 }}>
      ...
      <div
        ...
        style={{
          border: `2px dashed ${dragging ? '#2563eb' : entries.length >= MAX_FILES ? '#1e2d1e' : '#2a2d3a'}`,
          ...
          {entries.map(entry => {
            ...
            return (
              <div key={entry.id} style={{
                ...
                background: '#1a1d27', border: `1px solid ${entry.error ? '#3f1515' : entry.done ? '#1a3320' : '#2a2d3a'}`,
                borderRadius: 10, marginBottom: 8, transition: 'border-color 0.2s'
              }}>
                <div style={{ width: 40, height: 40, borderRadius: 8, background: color + '20', border: `1px solid ${color}40`, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 11, color: '#64748b', fontFamily: 'monospace' }}>{formatSize(entry.file.size)}</p>
      ...
      {entries.length > 0 && !allDone && (
        <button onClick={uploadAll} disabled={uploading || pendingCount === 0}
          style={{ width: '100%', padding: '14px', background: uploading ? '#1e3a5f' : pendingCount === 0 ? '#1a1d27' : '#2563eb', color: pendingCount === 0 && !uploading ? '#475569' : '#fff', border: `1px solid ${pendingCount === 0 && !uploading ? '#2a2d3a' : 'transparent'}`, borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: uploading || pendingCount === 0 ? 'default' : 'pointer' }}>
